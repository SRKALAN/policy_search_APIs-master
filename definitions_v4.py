...
# #     print(count,'FULLTEXT')
    if len(all_definitions)<10 and maxpage>10:
...
def convert_fitz_doc(file):
    ...
    for page in doc:
    #     print(page)
        text_lis.append((page.getText("text"),num))
        num+=1
    #     doc_text=doc_text+page.getText("text")+"\n"
    return text_lis
    

def def_extraction2(file):
    maxpage=0
    full_text=''
    def_page_lis=convert_fitz_doc(file)
    def_dict_1={}
    def_dict_2={}
    for line,page in def_page_lis:
        full_text+=line.replace('\n',' ')
...
#         print('def dict 2!!!!!',def_dict)
    ...
    count=len(re.findall(r"mean", full_text.lower()))
#     print(count,'FULLTEXT')
    if len(all_definitions)<10 and (maxpage>10 or count<4):
        all_definitions={}
        html_def_pages=html_extraction(file)
        for line,pgno in def_page_lis:
            if pgno in html_def_pages:
                all_definitions['Definitions '+str(pgno)]=(line.replace('\n',''),pgno)
#         print(all_definitions,'HTML')
#     all_definitions={}
    all_definitions={ ('no_name_found' if len(key)<3 else key):(value) for key, value in all_definitions.items() }
    return(all_definitions)